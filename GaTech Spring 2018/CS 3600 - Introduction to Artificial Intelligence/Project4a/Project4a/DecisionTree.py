...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for value in attrValues:
        dict = {}
        for example in examples:
            if example[attrName] == value:
                if example[className] in dict:
                    dict[example[className]] = dict[example[className]] + 1
                else:
                    dict[example[className]] = 1
        attributeCounts[value] = dict
...
def remainder(examples,attrName,attrValues,className):
    ...
    total = 0
    for key1 in attributeCounts.keys():
        for key2 in attributeCounts[key1].keys():
            total += attributeCounts[key1][key2]
    remainder = 0.0
    dict = {}
    for key1 in attributeCounts.keys():
        count = 0.0
        for key2 in attributeCounts[key1].keys():
            count += attributeCounts[key1][key2]
        dict[key1] = count
    for key1 in attributeCounts.keys():
        entropy = 0.0
        for key2 in attributeCounts[key1].keys():
            prob = attributeCounts[key1][key2] / dict[key1]
            entropy += prob * log(prob, 2)
        remainder += dict[key1] / total * (-entropy)
    return remainder
...
def giniGain(examples,attrName,attrValues,className):
    ...
    for key1 in attributeCounts.keys():
        ...
        classCount = []
        for key2 in attributeCounts[key1].keys():
            classCount.append(attributeCounts[key1][key2])
            count += attributeCounts[key1][key2]
        giniD += count / total * giniIndex(classCount)
    if giniD == 0:
        import sys
        return sys.maxint
    return 1 / giniD
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    for attribute in remainingAttributes:
        if gainFunc(examples, attribute, attributeValues[attribute], className) > maxgain:
            maxgain = gainFunc(examples, attribute, attributeValues[attribute], className)
            bestattr = attribute
    root = Node(bestattr)
    remAttributes = []
    for attribute in remainingAttributes:
        if attribute != bestattr:
            remAttributes.append(attribute)
    children = {}
    label = getMostCommonClass(examples, className)
    for value in attributeValues[bestattr]:
        newexamples = getPertinentExamples(examples, bestattr, value)
        subtree = makeSubtrees(remAttributes, newexamples, attributeValues, className, label, setScoreFunc, gainFunc)
        children[value] = subtree
