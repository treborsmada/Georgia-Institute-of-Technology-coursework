...
class Perceptron(object):
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        """YOUR CODE"""
        return self.sigmoidActivation(inActs) * (1 - self.sigmoidActivation(inActs))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        inActs = [1.0] + inActs
        index = 0
        while index < len(inActs):
            self.weights[index] = self.weights[index] + alpha * delta * inActs[index]
            totalModification += abs(alpha * delta * inActs[index])
            index += 1
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            list = []
            for per in layer:
                list.append(per.sigmoidActivation(layerlist[len(layerlist)-1]))
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    error = 0
                    ind = 0
                    while ind < len(nextLayer):
                        error += nextLayer[ind].weights[neuronNum + 1] * deltas[0][ind]
                        ind += 1
                    delta = error*gPrime
                    """FILL IN - delta for this neuron
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    testError = 0.0
    testCorrect = 0.0
    for exampleTest, exampleCorrect in examplesTest:
        testList = nnet.feedForward(exampleTest)[-1]
        correct = True
        index = 0
        while index < len(testList):
            if round(testList[index]) != exampleCorrect[index]:
                correct = False
            index += 1
        if correct:
            testCorrect += 1.0
